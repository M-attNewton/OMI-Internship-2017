% Calculates the fourier estimator covariance matrix
...
% Cycling throught different features
for h = selected_features
    ...
    for k = 1:round(nrfc)
        ...
        for i = 1:(size(DateTime_rescaled,1) - 1)
            ...
            terms_cos(i) = (cos(k*DateTime_rescaled(i)) - cos(k*DateTime_rescaled(i+1)))*Values_l(i,h);
            terms_sin(i) = (sin(k*DateTime_rescaled(i)) - sin(k*DateTime_rescaled(i+1)))*Values_l(i,h);
    ...
    var1 = (pi^2/(nrfc + 1 - n0))*...
        (sum(fca1(n0:round(nrfc)).*fca1(n0:round(nrfc))) + sum(fcb1(n0:round(nrfc)).*fcb1(n0:round(nrfc))));
    ...
    for g = selected_features
        ...
        for k = 1:round(nrfc)
            ...
            for i = 1:(size(DateTime_rescaled,1) - 1)
                ...
                terms_cos(i) = (cos(k*DateTime_rescaled(i)) - cos(k*DateTime_rescaled(i+1)))*Values_l(i,g);
                terms_sin(i) = (sin(k*DateTime_rescaled(i)) - sin(k*DateTime_rescaled(i+1)))*Values_l(i,g);
