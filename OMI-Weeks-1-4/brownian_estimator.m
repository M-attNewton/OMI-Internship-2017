% Uses T. Hayashi and N. Yoshidas method of modeling the behavoir as
% brownian motion, to obtain a cumulative covaraince matrix of returns.
...
for j = 1:size(Values_lr,2)
    ...
    for i = 1:size(Values_lr,1)
        ...
        if Values_lr(i,j) ~= 0 && Values_lr(i,j) ~= -Inf && Values_lr(i,j) ~= Inf && ~isnan(Values_lr(i,j)) 
            
            compare(n,2*j-1) = Values_lr(i,j);  %Value of interest           
...
% Compare each feature to one another by using the time intervals

for g = selected_features  %use of parfor to speed up computation
...
% The estimator can be standardised to find the correlation distribution

% Contains all the sigma terms corresponding to the feature
